...
const callback = async (message: MyMessage, args: string[]) => {
    ...
    if (user && requireUser) {
        ...
        if (Profile) {
            const fields = [];
            Profile.age !== 'none' ? fields.push({ name: 'Age', value: Profile.age, inline: true }) : null;
            Profile.gender !== 'none' ? fields.push({ name: 'Gender', value: Profile.gender, inline: true }) : null;
            Profile.pronouns !== 'none' ? fields.push({ name: 'Pronouns', value: Profile.pronouns, inline: true }) : null;

            message.client.sendEmbed(
                message,
                'Profiles',
                `${user.user.username}#${user.user.discriminator}`,
                Profile.biography !== 'none' ? `${Profile.biography}` : undefined,
